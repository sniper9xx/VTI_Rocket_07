...
public class flowControl {
	public static void main(String[] args) {
		...
		if (acc2.DepartID == null) {
			System.out.println("Nhân viên này chưa có phòng ban \n");
		} else {
			System.out.println("Phòng ban của nhân viên này là " + acc2.DepartID.DepartmentID + '\n');
		}

		// Question 2:
		System.out.println("Câu 2");
		int dem = 0;
		// gán accountid trong từng group bằng 1 số nào đó
		short a = gracc1.AccountID.AccountID;
		short b = gracc2.AccountID.AccountID;
		short c = gracc3.AccountID.AccountID;
		short d = gracc4.AccountID.AccountID;
		short e = gracc5.AccountID.AccountID;
		// tạo ra 1 mảng gồm các accountid của tất cả các group
		int arr[] = { a, b, c, d, e };
		...
		System.out.println(acc2.DepartID == null ? "Nhân viên này chưa có phòng ban \n"
				: "Phòng ban của nhân viên này là " + acc2.DepartID.DepartmentID + "\n");

		// Question 4:
		System.out.println("Câu 4");
		System.out
				.println(acc1.PosiID.PositionID == 1 ? "Đây là Developer \n" : "Người này không phải là Developer \n");

		// Switch Case
		// Question 5:
		System.out.println("Câu 5");
		byte count = 0;
		int x = gracc1.GroupID.GroupID; // Gán GroupID bằng 1 số
		int y = gracc2.GroupID.GroupID;
		int z = gracc3.GroupID.GroupID;
		int u = gracc4.GroupID.GroupID;
		int t = gracc5.GroupID.GroupID;
		int arr1[] = { x, y, z, u, t };
		...
		int dem6 = 0;
		// gán accountid trong từng group bằng 1 số nào đó
		short a6 = gracc1.AccountID.AccountID;
		short b6 = gracc2.AccountID.AccountID;
		short c6 = gracc3.AccountID.AccountID;
		short d6 = gracc4.AccountID.AccountID;
		short e6 = gracc5.AccountID.AccountID;
		// tạo ra 1 mảng gồm các accountid của tất cả các group
		int arr6[] = { a6, b6, c6, d6, e6 };
		...
		Account arrac[] = { acc1, acc2, acc3, acc4, acc5 };
		for (Account arac : arrac) {
			System.out.println("FullName : " + arac.FullName);
			System.out.println("Email : " + arac.Email);
			System.out.println("DepartmentName : " + arac.DepartID.DepartmentName + "\n");
		}

		// Question 9:
		System.out.println("Câu 9");
		Department departs[] = { depart1, depart2, depart3, depart4, depart5 };
		...
		for (int i = 0; i < arrac.length; i++) {
			...
			System.out.println("FullName : " + arrac[i].FullName);
			System.out.println("Email : " + arrac[i].Email);
			System.out.println("DepartmentName : " + arrac[i].DepartID.DepartmentName + "\n");
		}
		...
		for (int i = 0; i < departs.length; i++) {
			...
			System.out.println("ID = " + departs[i].DepartmentID);
			System.out.println("Name = " + departs[i].DepartmentName + "\n");
		}
		...
		for (int i = 0; i < arrac.length; i++) {
			if( arrac[i].DepartID.DepartmentID < 4){
			System.out.println("Thông tin account thứ " + (i + 1) + " là");
			System.out.println("FullName : " + arrac[i].FullName);
			System.out.println("Email : " + arrac[i].Email);
			System.out.println("DepartmentName : " + arrac[i].DepartID.DepartmentName + "\n");
		...
		while(i < arrac.length){
			...
			// 16 - 11:
		int k = 0;
		while( k < departs.length){
			System.out.println("Thông tin department thứ " + (k + 1) + " là");
			System.out.println("ID = " + departs[k].DepartmentID);
			System.out.println("Name = " + departs[k].DepartmentName + "\n");
			k++;
		}
			// 16 - 12
		int m = 0;
		while( m < 2){
			System.out.println("Thông tin department thứ " + (m + 1) + " là");
			System.out.println("ID = " + departs[m].DepartmentID);
			System.out.println("Name = " + departs[m].DepartmentName + "\n");
			m++;
		}
		
			// 16 -13
		int n = 0;
		while(n < arrac.length){
			if(n != 1){
			System.out.println("Thông tin account thứ " + (n + 1) + " là");
			System.out.println("FullName : " + arrac[n].FullName);
			System.out.println("Email : " + arrac[n].Email);
			System.out.println("DepartmentName : " + arrac[n].DepartID.DepartmentName + "\n");
			}
			n++;
		}
			...
			// 16 - 14
		int q = 0;
		while (q < arrac.length){
			if( arrac[q].DepartID.DepartmentID < 4){
			System.out.println("Thông tin account thứ " + (q + 1) + " là");
			System.out.println("FullName : " + arrac[q].FullName);
			System.out.println("Email : " + arrac[q].Email);
			System.out.println("DepartmentName : " + arrac[q].DepartID.DepartmentName + "\n");
			}
			q++;
		}	
